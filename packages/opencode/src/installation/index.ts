...
export const layer: Layer.Layer<Service, never, HttpClient.HttpClient | AppProcess.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getBrewFormula = Effect.fnUntraced(function* () {
      const tapFormula = yield* text(["brew", "list", "--formula", "leduyphuc1702/tap/opencode"])
      if (tapFormula.includes("opencode")) return "leduyphuc1702/tap/opencode"
    ...
    const result: Interface = {
      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        const response = yield* httpOk.execute(
          HttpClientRequest.get("https://api.github.com/repos/leduyphuc1702/opencode-workflow/releases/latest").pipe(
            HttpClientRequest.acceptJson,
          ),
        )
      ...
      upgrade: Effect.fn("Installation.upgrade")(function* (m: Method, target: string) {
        ...
        switch (m) {
          ...
          case "brew": {
            ...
            if (formula.includes("/")) {
              const tap = yield* run(["brew", "tap", "leduyphuc1702/tap"], { env })
              if (tap.code !== 0) {
                upgradeResult = tap
                break
              }
              const repo = yield* text(["brew", "--repo", "leduyphuc1702/tap"])
              const dir = repo.trim()
              if (dir) {
