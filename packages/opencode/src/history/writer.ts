    ...
    const state = yield* InstanceState.make<{ started: boolean }>(
      Effect.fn("History.Writer.state")(function* (_ctx) {
        ...
        yield* bus.subscribeCallback(MessageV2.Event.PartUpdated, (evt) => {
          // The event payload is deeply-readonly (Schema-decoded); extract() only
          // reads fields, so widen to the mutable Part type at the boundary.
          Queue.offerUnsafe(queue, { type: "upsert", part: evt.properties.part as MessageV2.Part, time: evt.properties.time })
        })
