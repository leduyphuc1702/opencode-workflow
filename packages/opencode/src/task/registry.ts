...
import { Database, and, eq, isNull, or, gt, type SQL } from "@/storage/db"
import { Bus } from "@/bus"
...
export const layer: Layer.Layer<Service, never, Bus.Service | Config.Service> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // Publish is awaited (yield*) at each call site rather than detached: the
    // Bus reads the ambient instance context, which a detached Effect.runFork
    // would not carry. PubSub publish is synchronous, so this is cheap.
    const publishCreated = (task: Task) => bus.publish(TaskCreated, { sessionID: task.session_id, task })
    const publishUpdated = (task: Task, kind: UpdatedKind) =>
      bus.publish(TaskUpdated, { sessionID: task.session_id, task, kind })
    ...
    const list = Effect.fn("TaskRegistry.list")(function* (input: {
      ...
      const rows = Database.use((db) => db.select().from(TaskTable).where(where).orderBy(TaskTable.created_at).all())
