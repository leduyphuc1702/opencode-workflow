 ...
 * is cleared on instance teardown.
 */
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const evaluate = Effect.fn("SessionGoal.evaluate")(function* (input: {
      ...
      // orDie model resolution so this Effect's error channel stays `never`; the
      // caller (goalGate) fail-opens on the whole cause (failures AND defects).
      ...
      if (isOpenaiOauth) {
        return yield* Effect.promise(async () => {
          ...
          return (await result.object) as Verdict
        })
      }
